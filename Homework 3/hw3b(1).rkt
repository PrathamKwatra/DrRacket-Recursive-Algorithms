...
(module+ test
  ...
  (check-equal? (dzero? '(DP1 zero)) #f "dzero? implementation")
  )
;helper function double
;input-spec: arity of 1, D-DP1 number
;output-spec: Doubles the number if not zero
(define (doubDB N)
...
(define (dsuccessor L)
 (cond [(dzero? L) '(DP1 zero)]
        [(equal? (first L) 'D) (cons 'DP1 (rest L))]
        [(doubDB (dsuccessor (second L)))]
  )
)
(module+ test
  (check-equal? (dsuccessor 'zero) '(DP1 zero) "dsuccessor implementation")
  (check-equal? (dsuccessor '(DP1 zero)) '(D (DP1 zero)) "dsuccessor implementation")
  (check-equal? (dsuccessor '(D (DP1 zero))) '(DP1 (DP1 zero)) "dsuccessor implementation")
  (check-equal? (dsuccessor '(D (D (DP1 zero)))) '(DP1 (D (DP1 zero))) "dsuccessor implementation")
  )
...
(define (predecessor L)
  (cond [(equal? '(DP1 zero) L) (getZero)]
        [(equal? (first L) 'D) (dsuccessor (doubDB (predecessor (second L))))]; (+1 (*2 (-1 (List[1:])))
        [(doubDB (second L))]
       )
  )
...
(module+ test
  ...
  (check-equal? (predecessor '(D (DP1 zero))) '(DP1 zero)  "predecessor implementation")
  (check-equal? (predecessor '(DP1 (DP1 zero))) '(D (DP1 zero))  "predecessor implementation")
  )
...
(module+ test
  (check-equal? (addD '(DP1 zero) '(DP1 zero)) '(D (DP1 zero)) "addD implementation")
  (check-equal? (addD '(D (DP1 zero)) '(DP1 zero)) '(DP1 (DP1 zero))  "addD implementation")
  (check-equal? (addD '(D (DP1 zero)) '(DP1 (D (DP1 zero)))) '(DP1 (DP1 (DP1 zero))) "addD implementation")
  )
...
(define (L<M? L M)
  (cond
    [(equal? M 'zero)    #f]
    [(equal? L 'zero)    #t]
    [(equal? (second L) (second M))    (and (equal? (first L) 'D) (equal? (first M) 'DP1))]
    [else    (L<M? (second L) (second M))])
  )
...
(module+ test
  (check-equal? (subD '(DP1 zero) '(DP1 zero)) 'zero "subD implementation")
  (check-equal? (subD '(D (DP1 zero)) '(DP1 zero)) '(DP1 zero)  "subD implementation")
  (check-equal? (subD '(D (DP1 zero)) '(DP1 (D (DP1 zero)))) '(DP1 (DP1 zero)) "subD implementation"))


; remainder of division between two D-DP1 numbers
; Input: D-DP1 numbers L1, L2, also L1 is always bigger than L2 
; Output: the remainder of the division between L1 and L2
(define (remainderD L1 L2)
  (cond [(equal? L1 'zero) 'zero]
        [(equal? L2 'zero) 'zero]
  [(if (L<M? L1 L2) L1 (remainderD (subD L1 L2) L2))]))

(module+ test
  (check-equal? (remainderD '(DP1 zero) '(DP1 zero)) 'zero "remainderD implementation")
  (check-equal? (remainderD '(D (DP1 zero)) '(D (D (DP1 zero)))) '(D (DP1 zero))  "remainderD implementation")
  (check-equal? (remainderD '(D (D (DP1 zero))) '(D (DP1 zero))) 'zero "remainderD implementation")
  (check-equal? (remainderD '(DP1 (DP1 (DP1 zero))) '(DP1 (DP1 zero))) '(DP1 zero) "remainderD implementation")
  )

; rounded-down division between two D-DP1 numbers
; Input: D-DP1 numbers L1, L2, L1 > L2 
; Output: the floor of the division between L1 and L2
(define (quotientD L1 L2)
  (if (L<M? L1 L2) 'zero (addD '(DP1 zero) (quotientD (subD L1 L2) L2)))
  )
(module+ test
  (check-equal? (quotientD '(DP1 zero) '(DP1 zero)) '(DP1 zero) "quotientD implementation")
  (check-equal? (quotientD '(D (DP1 zero)) '(D (D (DP1 zero)))) 'zero "quotientD implementation")
  (check-equal? (quotientD '(D (D (DP1 zero))) '(D (D (DP1 zero)))) '(DP1 zero) "quotientD implementation")
  (check-equal? (quotientD '(DP1 (DP1 (DP1 zero))) '(D (D (DP1 zero)))) '(DP1 zero) "quotientD implementation")
  (check-equal? (quotientD '(D (DP1 (DP1 zero))) '(D (D (DP1 zero)))) '(DP1 zero) "quotientD implementation")
  (check-equal? (quotientD '(D (D (D (D (DP1 zero))))) '(D (D (DP1 zero)))) '(D (D (DP1 zero))) "quotientD implementation"))
...
(module+ test
  (check-equal? (divides? '(DP1 zero) '(DP1 zero)) #t "divides? implementation")
  (check-equal? (divides? '(D (DP1 zero)) '(D (D (DP1 zero)))) #f "divides? implementation")
  ...
  (check-equal? (divides? '(D (D (D (D (DP1 zero))))) '(D (D (DP1 zero)))) #t "divides? implementation")
  (check-equal? (divides? '(D (DP1 (DP1 zero))) '(D (D (DP1 zero)))) #f "divides? implementation"))
...
(module+ test
  (check-equal? (gcdD  '(DP1 (DP1 (DP1 (DP1 zero)))) '(D (D (D (DP1 zero))))) '(DP1 zero) "gcdD implementation")
  ...
  (check-equal? (gcdD  '(DP1 (D (DP1 (DP1 zero)))) '(DP1 (D (DP1 (DP1 zero))))) '(DP1 (D (DP1 (DP1 zero)))) "gcdD implementation")
  (check-equal? (gcdD  'zero 'zero) 'zero "gcdD implementation")
)
